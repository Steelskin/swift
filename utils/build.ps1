  ...
  Invoke-IsolatingEnvVars {
    ...
    if (Test-Path $CMakeCacheFile) {
      ...
      foreach ($Define in $ParsedDefines.GetEnumerator()) {
        if ($CMakeCache.ContainsKey($Define.Key) -and $CMakeCache[$Define.Key] -ne $Define.Value) {
      ...
      if (-not $NeedsConfigure) {
        ...
        if ($CMakeCache.ContainsKey("CMAKE_GENERATOR") -and $CMakeCache["CMAKE_GENERATOR"] -ne $Generator) {
    ...
    if ($NeedsConfigure) {
      ...
      if ($ToBatch) {
        Write-Output ""
        Write-Output "echo Skipping reconfiguration of '$Src' to '$Bin' as nothing changed."
      } else {
        Write-Host -ForegroundColor Yellow "Skipping reconfiguration of '$Src' to '$Bin' as nothing changed."
      }
    }
